...
export const onOrderMakeStart = (data: ProductsProps | undefined, cartItem: {id: number, quantity: number, section: string}[]) => {
	...
	const items = data?.data.products.map((item) => {
		...
		return {
			...
			item_category: cart?.section === 'avtoshini' ? 'Автошини' : cart?.section === 'diski' ? 'Автодиски' : cart?.section === 'akumulyatori' ? 'Акумулятори' : '',
...
export const onOrderBuy1click = (
	...
	const items = [{
		...
		item_brand: brand,
		item_category: section === 'avtoshini' ? 'Автошини' : section === 'diski' ? 'Автодиски' : section === 'akumulyatori' ? 'Акумулятори' : '',
		item_variant: model,
		quantity: quantity,
	}];
